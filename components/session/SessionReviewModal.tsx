          ...
          <div className="mb-6">
            <label className="flex cursor-pointer items-center gap-3 rounded-xl px-4 py-3" style={{ background: "rgba(255,255,255,0.04)" }}>
              <input
                ...
                className="h-4 w-4 rounded accent-[var(--sg-forest-400)]"
              />
              <span className="flex-1 text-sm text-[var(--sg-shell-300)]">
                Post progress note to{" "}
                <span className="capitalize text-white">{linkedResource.provider}</span>
              </span>
              {writebackSending && (
                <span className="text-xs text-[var(--sg-shell-500)]">Sending...</span>
              )}
            </label>
          </div>
        )}
